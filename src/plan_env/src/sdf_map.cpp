 ...
 *                   and fast_planner (https://github.com/HKUST-Aerial-Robotics/fast_planner).
...
void SDFMap::initHCMap(ros::NodeHandle& nh, pcl::PointCloud<pcl::PointXYZ>::Ptr& model)
{
  ...
  if (!std::isfinite(hcmp_->resolution_) || hcmp_->resolution_ <= 1e-3)
  {
    ROS_WARN("[HCMap] Invalid hcmap/resolution=%.3f, fallback to 0.40", hcmp_->resolution_);
    hcmp_->resolution_ = 0.40;
  }
  if (!std::isfinite(hcmp_->proj_interval) || hcmp_->proj_interval <= 1e-3)
  {
    ROS_WARN("[HCMap] Invalid hcmap/interval=%.3f, fallback to 1.00", hcmp_->proj_interval);
    hcmp_->proj_interval = 1.00;
  }
  if (!std::isfinite(hcmp_->thickness) || hcmp_->thickness <= 1e-3)
  {
    ROS_WARN("[HCMap] Invalid hcmap/plane_thickness=%.3f, fallback to 3.00", hcmp_->thickness);
    hcmp_->thickness = 3.00;
  }
  if (!std::isfinite(hcmp_->checkScale) || hcmp_->checkScale <= 1e-3)
  {
    ROS_WARN("[HCMap] Invalid hcmap/checkScale=%.3f, fallback to 5.00", hcmp_->checkScale);
    hcmp_->checkScale = 5.00;
  }
  if (checkSize <= 0)
  {
    ROS_WARN("[HCMap] Invalid hcmap/checkSize=%d, fallback to 7", checkSize);
    checkSize = 7;
  }
  if (inflate_num < 0)
  {
    ROS_WARN("[HCMap] Invalid hcmap/inflateVoxel=%d, fallback to 4", inflate_num);
    inflate_num = 4;
  }
  if (!std::isfinite(hcmp_->size_inflate) || hcmp_->size_inflate <= 0.0)
  {
    ROS_WARN("[HCMap] Invalid viewpoint_manager/visible_range=%.3f, fallback to 3.00", hcmp_->size_inflate);
    hcmp_->size_inflate = 3.00;
  }
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr finite_model(new pcl::PointCloud<pcl::PointXYZ>);
  if (model)
  {
    finite_model->points.reserve(model->points.size());
    for (const auto& pt : model->points)
    {
      if (!std::isfinite(pt.x) || !std::isfinite(pt.y) || !std::isfinite(pt.z))
        continue;
      finite_model->points.push_back(pt);
    }
  }
  if (finite_model->points.empty())
  {
    ROS_WARN("[HCMap] Input model has no finite points, fallback to a minimal map around origin.");
    finite_model->points.push_back(pcl::PointXYZ(0.0f, 0.0f, 0.0f));
  }
  
  /* find scene size */
  pcl::PointXYZ pt_min;
  pcl::PointXYZ pt_max;
  pcl::getMinMax3D(*finite_model, pt_min, pt_max);
  ...
  double x_size, y_size, z_size;
  x_size = std::max(hcmp_->resolution_, 4 * hcmp_->size_inflate + pt_max.x - pt_min.x);
  y_size = std::max(hcmp_->resolution_, 4 * hcmp_->size_inflate + pt_max.y - pt_min.y);
  z_size = std::max(hcmp_->resolution_, 4 * hcmp_->size_inflate + pt_max.z - pt_min.z);
  ...
  const size_t nx = static_cast<size_t>(std::max(1, hcmp_->map_voxel_num_(0)));
  const size_t ny = static_cast<size_t>(std::max(1, hcmp_->map_voxel_num_(1)));
  const size_t nz = static_cast<size_t>(std::max(1, hcmp_->map_voxel_num_(2)));
  const size_t max_cells_safe = 150000000UL;

  auto apply_conservative_cube = [&]() {
    hcmp_->map_voxel_num_ = Eigen::Vector3i(256, 256, 128);
    hcmp_->map_size_ = hcmp_->map_voxel_num_.cast<double>() * hcmp_->resolution_;
    hcmp_->map_min_boundary_ = hcmp_->map_origin_;
    hcmp_->map_max_boundary_ = hcmp_->map_origin_ + hcmp_->map_size_;
    hcmp_->box_mind_ = hcmp_->map_min_boundary_;
    hcmp_->box_maxd_ = hcmp_->map_max_boundary_;
  ...
  size_t buffer_size = nx;
  if (ny > std::numeric_limits<size_t>::max() / buffer_size)
  {
    ROS_WARN("[HCMap] Voxel dimension overflow (nx=%zu, ny=%zu), fallback to a conservative cube.", nx, ny);
    apply_conservative_cube();
    buffer_size = static_cast<size_t>(hcmp_->map_voxel_num_(0)) *
                  static_cast<size_t>(hcmp_->map_voxel_num_(1)) *
                  static_cast<size_t>(hcmp_->map_voxel_num_(2));
  }
  else
  {
    buffer_size *= ny;
    if (nz > std::numeric_limits<size_t>::max() / buffer_size)
    {
      ROS_WARN("[HCMap] Voxel dimension overflow (nxy=%zu, nz=%zu), fallback to a conservative cube.", buffer_size, nz);
      apply_conservative_cube();
      buffer_size = static_cast<size_t>(hcmp_->map_voxel_num_(0)) *
                    static_cast<size_t>(hcmp_->map_voxel_num_(1)) *
                    static_cast<size_t>(hcmp_->map_voxel_num_(2));
  ...
  if (buffer_size > max_cells_safe)
  {
    ROS_WARN("[HCMap] Too many cells (%zu), fallback to a conservative cube.", buffer_size);
    apply_conservative_cube();
    buffer_size = static_cast<size_t>(hcmp_->map_voxel_num_(0)) *
                  static_cast<size_t>(hcmp_->map_voxel_num_(1)) *
                  static_cast<size_t>(hcmp_->map_voxel_num_(2));
  ...
  hcmd_->occupancy_buffer_hc_.assign(buffer_size, 0);
  hcmd_->occupancy_inflate_buffer_hc_.assign(buffer_size, 0);
  hcmd_->occupancy_buffer_internal_.assign(buffer_size, 0);

  internal_cast_.reset(new RayCaster);
  internal_cast_->setParams(hcmp_->resolution_, hcmp_->map_origin_);

  for (const auto& occ : finite_model->points)
  {
    Eigen::Vector3d pt_occ;
    Eigen::Vector3i id_occ;
    int adr_occ;
    pt_occ << occ.x, occ.y, occ.z;
    if (!isInMap_hc(pt_occ))
      continue;
  ...
  ROS_INFO("[HCMap] Initialized: finite_pts=%zu, resolution=%.3f, voxels=(%d,%d,%d), cells=%zu",
           finite_model->points.size(), hcmp_->resolution_,
           hcmp_->map_voxel_num_(0), hcmp_->map_voxel_num_(1), hcmp_->map_voxel_num_(2), buffer_size);
}
