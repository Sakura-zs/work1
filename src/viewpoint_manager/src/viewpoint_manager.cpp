...
namespace fast_planner
{
  ...
  void ViewpointManager::init(ros::NodeHandle &nh)
  {
    ...
    auto set_default_double = [&nh](const std::string &key, double value) {
      if (!nh.hasParam(key))
      {
        nh.setParam(key, value);
      }
    };
    auto set_default_int = [&nh](const std::string &key, int value) {
      if (!nh.hasParam(key))
      {
        nh.setParam(key, value);
      }
    ...
    // HC map params are required by setMapPointCloud(). In EPIC integration path these
    // params may be absent, so provide safe defaults.
    set_default_double("hcmap/resolution", 0.40);
    set_default_double("hcmap/interval", 1.00);
    set_default_double("hcmap/plane_thickness", 3.00);
    set_default_double("hcmap/checkScale", 5.00);
    set_default_int("hcmap/checkSize", 7);
    set_default_int("hcmap/inflateVoxel", 4);
    ...
    // PerceptionUtils also depends on dedicated params that are not always loaded by
    // exploration_node launch files.
    const double fov_h_half_deg = std::max(5.0, std::min(89.0, 0.5 * fov_h));
    const double fov_w_half_deg = std::max(5.0, std::min(89.0, 0.5 * fov_w));
    const double fallback_range = std::max(2.0, std::max(visible_range, dist_vp + 0.5));
    set_default_double("perception_utils/top_angle", fov_h_half_deg * M_PI / 180.0);
    set_default_double("perception_utils/left_angle", fov_w_half_deg * M_PI / 180.0);
    set_default_double("perception_utils/right_angle", fov_w_half_deg * M_PI / 180.0);
    set_default_double("perception_utils/max_dist", fallback_range);
    set_default_double("perception_utils/vis_dist", std::max(1.0, std::min(3.0, fallback_range)));
  ...
  void ViewpointManager::updateViewpoints()
  {
    if (!TS.viewpoints_flag_)
    {
      ...
      for (int i = 0; i < (int)model_->points.size(); i++)
      {
        ...
        auto normal_it = MCI.pt_normal_pairs.find(pt_vec);
        if (normal_it == MCI.pt_normal_pairs.end())
          continue;
        const auto &normal_dir = normal_it->second;
    ...
    // vps_voxcount/vps_contri are indexed by viewpoint id, so keep them
    // aligned with viewpoint count rather than model point count.
    const size_t vp_count = static_cast<size_t>(VPI.vps_pose.rows());
    if (temp_vps_voxcount.size() < vp_count)
      temp_vps_voxcount.resize(vp_count, 0);
    VPI.vps_voxcount.assign(vp_count, 0);
    if (VPI.vps_contri.size() < vp_count)
      VPI.vps_contri.resize(vp_count, 0);
    ...
    for (int i = 0; i < (int)uncoveredID.size(); ++i)
    {
      ...
      if (normal_it == MCI.pt_normal_pairs.end())
        continue;
      const Vector3d normal_dir = normal_it->second;
  ...
  bool ViewpointManager::viewpointSafetyCheck(pcl::PointXYZ &vp)
  {
    ...
    const double map_resolution = HCMap->hcmp_->resolution_;
    if (!std::isfinite(map_resolution) || map_resolution <= 1e-6)
    {
      ROS_WARN_THROTTLE(1.0, "[ViewpointManager] Invalid HC map resolution for safety check: %.6f", map_resolution);
      return false;
    }

    Eigen::Vector3d safe_check_pt;
    int vox_num = 2;
    const int safe_voxel = std::max(1, static_cast<int>(floor(0.5 * safe_radius_vp / map_resolution)));
    for (int i = 0; vp.x - safe_radius_vp + i * map_resolution < vp.x + safe_radius_vp; i = i + safe_voxel)
      for (int j = 0; vp.y - safe_radius_vp + j * map_resolution < vp.y + safe_radius_vp; j = j + safe_voxel)
        for (int k = 0; vp.z - safe_radius_vp + k * map_resolution < vp.z + safe_radius_vp; k = k + safe_voxel)
        {
          safe_check_pt(0) = vp.x - safe_radius_vp + i * map_resolution;
          safe_check_pt(1) = vp.y - safe_radius_vp + j * map_resolution;
          safe_check_pt(2) = vp.z - safe_radius_vp + k * map_resolution;
  ...
  bool ViewpointManager::nearCheck(Eigen::Vector3d &cur_position, int &vox_num)
  {
    ...
    try
    {
      found_num = map_cloud_kdtree_.nearestKSearch(search_point, knn, nearest_id, nn_squared_distance);
    }
    ...
    if (found_num <= 0 || nn_squared_distance.empty())
      return false;
