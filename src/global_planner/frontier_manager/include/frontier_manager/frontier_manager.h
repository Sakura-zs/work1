...
struct SupplementarySsdCache {
  bool valid = false;
  ros::Time stamp;
  Eigen::Vector3f center = Eigen::Vector3f::Zero();
  int input_cloud_size = 0;

  vector<Eigen::Vector3d> input_cloud;
  vector<Eigen::Vector3d> skeleton_vertices;
  vector<Eigen::Vector2i> skeleton_edges;
  vector<vector<int>> branches;
  map<int, vector<Eigen::Vector3d>> segment_clouds;
  vector<vector<Eigen::Vector3d>> subspace_clouds;
  vector<SubspaceFitResult> subspace_fit_results;
};
...
private:
  ...
  ros::NodeHandle nh_;
  SupplementarySsdCache supplementary_ssd_cache_;
  ros::Subscriber accum_cloud_sub_;
  pcl::PointCloud<pcl::PointXYZ> accum_cloud_;
  bool has_accum_cloud_ = false;
  std::mutex accum_cloud_mtx_;
  ros::Time last_ssd_time_;  // 防止重复计算同一位置
  ...
  void publishSupplementarySsdCache();
  void accumCloudCallback(const sensor_msgs::PointCloud2ConstPtr &msg);
...
public:
  ...
  void updateFrontierClusters(vector<ClusterInfo::Ptr> &cluster_updated,
                              vector<int> &cluster_removed);
  void generateSupplementaryViewpoints(Eigen::Vector3f &center_pose,
                                       vector<TopoNode::Ptr> &viewpoints);
  void generateTSPViewpoints(Eigen::Vector3f &center_pose,
                             vector<TopoNode::Ptr> &viewpoints);

  const SupplementarySsdCache &getSupplementarySsdCache() const {
    return supplementary_ssd_cache_;
  }

  bool hasSupplementarySsdCache() const { return supplementary_ssd_cache_.valid; }
};