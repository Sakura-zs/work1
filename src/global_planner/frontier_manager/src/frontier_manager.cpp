...
void FrontierManager::init(ros::NodeHandle &nh, LIOInterface::Ptr &lio_interface,
  ...
  string rosa_cloud_accum_topic;
  nh.param<string>("frontier/rosa_cloud_accum_topic", rosa_cloud_accum_topic,
                   "/quad0_pcl_render_node/cloud_accum");
  accum_cloud_sub_ = nh_.subscribe<sensor_msgs::PointCloud2>(
      rosa_cloud_accum_topic, 1, &FrontierManager::accumCloudCallback, this);
  ROS_INFO("[FRONTIER] ROSA cloud source topic: %s", rosa_cloud_accum_topic.c_str());
}

void FrontierManager::accumCloudCallback(
    const sensor_msgs::PointCloud2ConstPtr &msg) {
  pcl::PointCloud<pcl::PointXYZ> cloud;
  pcl::fromROSMsg(*msg, cloud);
  std::lock_guard<std::mutex> lock(accum_cloud_mtx_);

  // 方案一：累积仅在发布端执行；这里仅缓存最新累积云快照
  accum_cloud_ = std::move(cloud);

  has_accum_cloud_ = !accum_cloud_.points.empty();
}
