...
{ 
  ...
  void ROSA_main::branch_merge()
  {
    ...
    for (int i=0; i<(int)P.branches.size(); ++i)
    {
      int joint_endpoint = -1;
      int leaf_endpoint = -1;
      if (P.degrees(P.branches[i].front(),0) == 1 && P.degrees(P.branches[i].back(),0) > 1)
      {
        leaf_endpoint = P.branches[i].front();
        joint_endpoint = P.branches[i].back();
      }
      else if (P.degrees(P.branches[i].back(),0) == 1 && P.degrees(P.branches[i].front(),0) > 1)
      {
        leaf_endpoint = P.branches[i].back();
        joint_endpoint = P.branches[i].front();
      }
      ...
      if (leaf_endpoint >= 0 && joint_endpoint >= 0 && P.degrees(joint_endpoint,0) >= 3)
      {
        continue;
      }
